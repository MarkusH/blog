...
class Gallery(Directive):
    ...
    def run(self):
        ...
        max_widths = {"s": 500.594, "m": 768.688, "l": 1007.906}
        ...
        for img in images:
            ...
            for key in ("s", "m", "l"):
                ...
                # Create the CSS classes, e.g. s12, m6, l4
...
class Speakerdeck(Directive):
    required_arguments = 1
    option_spec = {"ratio": directives.unchanged}
    has_content = False

    def run(self):
        node = speakerdeck(
            key=self.arguments[0], ratio=self.options.get("ratio", "1.3333333333")
        )
        return [node]
