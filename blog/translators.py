...
class GalleryTranslator:
    def visit_gallery_node(self, node):
        ...
        for image in node.images:
            ...
            self.body.append(
                '<img class="large lazyload" data-src="/images/thumb/%s" '
                'alt="%s" title="%s">' % (image.thumbs[2], image["alt"], image["alt"])
            )
            self.body.append(
                '<img class="medium lazyload" data-src="/images/thumb/%s" '
                'alt="%s" title="%s">' % (image.thumbs[1], image["alt"], image["alt"])
            )
            self.body.append(
                '<img class="small lazyload" data-src="/images/thumb/%s" '
                'alt="%s" title="%s">' % (image.thumbs[0], image["alt"], image["alt"])
            )
            self.body.append("</a>")
...
class ProjectTranslator:
    ...
    def _depart_project_item(self, node):
        start = (len(self.body) - 1) - self.body[::-1].index(
        ...
        self.body.append("</div>")
        self.body.append(
            self.starttag(node, "div", CLASS="collapsible-body hide-on-large-only")
        )
        self.body.append(self.starttag(node, "p"))
...
class BlogHTMLTranslator(
    GalleryTranslator,
    ProjectTranslator,
    PNGMathTranslator,
    SpeakerdeckTranslator,
...
class AMPTranslator(BlogHTMLTranslator):
    ...
    def visit_image(self, node):
        ...
        if last.startswith("<img "):
            kwargs = {"LAYOUT": "fixed", "SRC": node["uri"]}
            self.body.pop()
            src = node["uri"]
            if src.startswith("/"):
            ...
            raise AssertionError(
                "Why's there no <img> tag as last element on the stack?"
            )
    ...
    def visit_docinfo(self, node):
        ...
        self.body.append(
            '<col class="docinfo-name" /><col class="docinfo-content" /><tbody>'
        )
